...
const gameCorePath = "src/legacy/gameCore.js";
let gameCore = fs.readFileSync(gameCorePath, "utf8");
gameCore = replaceOnce(
  gameCore,
...
gameCore = replaceOnce(
  ...
  `export function runReportOpenBudget() {\n  gameState.phase = "budget";\n  setScreen("budget");\n}\n${resultViewModelBlock}\nfunction createRegionalMoods() {`,
  "gameCore insert result view model/actions",
);
fs.writeFileSync(gameCorePath, gameCore);

const screensPath = "src/components/GameScreens.jsx";
...
`;
if (screens.includes("export function ResultScreenShell")) {
  throw new Error("ResultScreenShell already exists");
}
screens = `${screens.trimEnd()}\n\n${resultComponent}`;
fs.writeFileSync(screensPath, `${screens}\n`);

console.log("B-1 result react migration applied");
